...
│ 戒律 6: 严守包边界与测试驱动   │ 核心包与表现层物理隔离，所有检测规则必须包含正反单测 │
└───────────────────────────────┴─────────────────────────────────────────────┘
```
...
### 戒律 6：严守工程分层边界与 100% 规则测试 (Layer Invariants & TDD)
- **模块分层守则**：
...
当接到开发任务时，Agent 请按如下顺序开展工作：
1. **核对约束**：查阅当前任务是否违反上述六大戒律，严禁越界引入重型依赖；
2. **测试先行**：在 `@sextant/core/tests` 中先编写违规代码与合规代码的 AST 提取测试用例（红灯）；
3. **极简实现**：编写高内聚、纯函数的解析与差分逻辑，使测试通过（绿灯）；
4. **自检门禁**：运行完整测试套件，确保零回归、执行时间无膨胀；
5. **拒绝过度工程**：不添加未经确认的复杂配置或外部框架，保持代码整洁纯粹。
