...
### 戒律 4：零侵入开放协议，严禁自立私有孤岛标准
- **背景**：开发者不会为了一个工具重写整套规范，机器门禁亦不可依赖易碎模糊的散文格式。
- **约束**：
  - **总线单源与开放媒介**：以代码仓库中 Git 纳管的标准结构化配置（`sextant.json`，配合 JSON Schema 强校验）为唯一事实单源，并无缝兼容与自动同步 Markdown + Mermaid + YAML 只读视图；
