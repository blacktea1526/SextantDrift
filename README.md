...
### 2.3 100% 离线自包含原生 SVG 审查报告
- **零外部 CDN 依赖**：生成的 `drift-report.html` 为完全自包含的单一 HTML 文件，**不发任何外部网络请求、不加载任何外部 JS/CSS/字体 CDN**；
- **内网与气隙环境开箱即用**：金融、军工及企业私网完全安全隔离；
- **工业级交互体验**：内置原生 SVG 渲染，支持 C4 Level 2（容器）与 Level 3（组件）层级切换、无限画布平移缩放 (Pan & Zoom)、契约连接线过滤、违规高亮聚焦与中英双语即时切换。
...
- **覆盖率与完备性**：内置 44 个测试套件、229 个测试用例，涵盖各类边界情况与极端图拓扑，代码质量经久耐用。
...
#### 步骤 5：交互式双图可视化审查报告 (`report`)
当需要直观审阅系统全局架构、或向团队成员汇报时，生成原生的双图红绿审查报告：

```bash
# 生成自包含离线报告 (默认输出 drift-report.html)
npx sextant-drift report

# 支持指定输出中英语言 (默认 zh，可切换 en)
...
双击在任何浏览器打开 `drift-report.html` 即可享受：
...
./start.sh --test     # 执行全套单元测试 (Vitest CLI, 44 个套件, 229 个用例全部通过)
...
│   ├── cli/            # @sextant/cli: 极轻量命令行工具 (基于 cac + picocolors, < 50KB, 零冗余依赖)
│   └── web-report/     # @sextant/web-report: 100% 离线自包含的原生 SVG 双图审查报告生成器
